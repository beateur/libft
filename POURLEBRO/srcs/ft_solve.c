...
/*   Updated: 2019/01/26 22:22:47 by bihattay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		if_tetris_fits(t_map *map, int row, int col, t_tetris *piece)
{
	printf("\n\n\n passage : [%d]\n\n", COMPTEUR++);
//	if (map->set[row][col])
		printf("/*************** 1st is ||  set[%c] row/col[%d || %d]  || **************\\\n", map->set[row][col], row, col);
	if (map->set[row][col] != '.')
		return (0);
	row += piece->r1;
	col += piece->c1; // quand col < 2 et que piece->c1 = -2 on obtient un col negatif et c'est pas cool
//	if (map->set[row][col])
		printf("/*************** 2nd is ||  set[%c] row/col[%d || %d]  || **************\\\n", map->set[row][col], row, col);
	if (map->set[row][col] != '.')
		return (0);
	row += piece->r2;
	col += piece->c2;
//	if (map->set[row][col])
		printf("/*************** 3rd is ||  set[%c] row/col[%d || %d]  || **************\\\n", map->set[row][col], row, col);
	if (map->set[row][col] != '.')
		return (0);
	row += piece->r3;
	col += piece->c3;
//	if (map->set[row][col])
		printf("/*************** 4th is ||  set[%c] row/col[%d || %d]  || **************\\\n", map->set[row][col], row, col);
	if (map->set[row][col] != '.')
		return (0);
	return (1);
}
